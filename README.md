...
The SSH key is per-machine and never leaves the machine -- nothing here syncs a
private key. The bootstrap task generates `~/.ssh/id_ed25519` if it is missing,
loads it into the macOS keychain, logs in to GitHub, and registers the key twice:
once for authentication and once for commit signing. It then writes
`~/.ssh/allowed_signers` from the account's signing keys and seeds GitHub's host
keys into `known_hosts` so the first clone does not stop on a prompt.

Per-machine rather than one synced key because there is exactly one consumer --
`~/.ssh/config` has no `Host` block but GitHub -- so registering a new key costs
one API call, while a lost laptop costs one `gh ssh-key delete` instead of a
rotation everywhere.

The key is generated with no passphrase. That is what keeps the install to a
single browser trip, and the trade-off is deliberate: the disk is encrypted at
rest, and the key is individually revocable. `ssh-keygen -p -f ~/.ssh/id_ed25519`
adds one later without changing the public key.

`user.name`, `user.email` and `user.signingkey` stay in the tracked
`~/.gitconfig`. None of them are machine-specific: the first two are the same
identity everywhere, and the third is a *path*, so it stays correct even though
every machine holds a different key behind it. Anything genuinely local belongs
in `~/.gitconfig.local`, which is included last and wins.

Every step is guarded, so re-running changes nothing. Without a terminal the
task prints what it skipped and the rest of the bootstrap still converges.
...
- Commits are SSH-signed. `~/.ssh/allowed_signers` is generated by the
  bootstrap task from every signing key on the GitHub account, so it is derived
  state rather than something to track or hand-edit.
